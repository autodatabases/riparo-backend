  ...
  logIn: async function (req, res) {
    try {
      ...
      if(user_data){
        ...
        if (!accountActive) return fun.returnResponse(res,false,401,"Your account has been temporarily deactivated. Please contact support team");
  ...
  addMembershipOption: async function (req, res) {
    try {
      ...
      if(!checkMembershipOption){
        ...
        if(insert_data){
          return fun.returnResponse(res,true,200,"Option Added Successfully");          
        }else{
          return fun.returnResponse(res,false,500,"Option cannot be inserted.");
        }

      } else {
        return fun.returnResponse(res,false,401,"Option already added with same title.");
        return false;
      }
    } catch (error) {
      throw error;
