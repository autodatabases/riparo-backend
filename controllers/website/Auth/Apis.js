...
module.exports = {
  ...
  signUp: async function (req, res) {
    try {
      ...
      req.check('userType').notEmpty().withMessage('Please enter your userType');
      ...
      if(user_data){
        return  fun.returnResponse(res,false,409,"Email is already registered with another account");
        return false;
      }
      ...
      if(insertData){
        ...
        if(requestData.userType === 'Customer'){
          ...
          var token = jwt.sign({ id: returnData.id }, process.env.API_SECRET, {
        ...
        return fun.returnResponse(res,true,200,"Account Created Successfully",returnData);
  ...
  forgotPassword: async function (req, res) {
    try {
      ...
      if(user_data){
        ...
        return fun.returnResponse(res,true,200,"Password reset link has been sent to your email.");
  ...
  resendCode: async function (req, res) {
    try {
      ...
      updateData =  await User.update({ verificationCode:code },{
                      where:{
                        id : req.id
                      }
                    });
  ...
  verifyCode: async function (req, res) {
    try {
      req.check('code').notEmpty().withMessage('Please enter verification code');
      var error = req.validationErrors();
      if (error) {
        return  fun.returnResponse(res,false,400,fun.checkRequired(error).message);
        return false;
      ...
      if(user_data){
        ...
        await User.update({ isEmailVerified:'Yes',emailVerifiedAt: await fun.getDate() },{
          where:{
            id : req.id
          }
        });
        ...
        return  fun.returnResponse(res,true,200,"Email Verified Successfully");
        return false;
      }else{
        return  fun.returnResponse(res,false,400,"Incorrect Verification Code");
        return false;
      }
    } catch (error) {
      throw error;
